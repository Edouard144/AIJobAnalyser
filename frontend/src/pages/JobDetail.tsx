...
export default function JobDetail() {
  ...
  const { getJob, updateJob, addCandidates, uploadCsv, runScreening } = useJobs();
  const job = getJob(id!);
  ...
  const saveEdit = async () => {
    try {
      await updateJob(job.id, { title: editTitle, description: editDesc, location: editLocation });
      setEditing(false);
  ...
  const handleRunScreening = async () => {
    ...
    setScreeningMsg(0);
    try {
      await runScreening(job.id, topN);
